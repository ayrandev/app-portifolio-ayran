...
const Cards = ({ title, description, className, image, Icon, Buttons}) => {
  return (
      ...
      <div className={`relative w-52 h-28 overflow-hidden rounded-lg shadow-lg group border-r-8 border-[#00FFEA] ${className}`}>
         <h1 className="w-full h-full items-center flex justify-start object-cover transition-transform duration-300 group-hover:translate-y-full font-[Poppins] text-2xl text-white">{title} {image} {Icon}</h1>
